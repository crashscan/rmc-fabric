...
static MonitorCallbacks makeCallbacks(
    INetworkObservationModel& model,
    LldpObserver* lldpObserver)
{
    MonitorCallbacks cb;
...
bool NetworkObservationAdapter::start() {
    model_->prepareForRestart();   // no-op on first start (empty candidates)
    ...
    model_->markLive();   // no more dynamic_cast
